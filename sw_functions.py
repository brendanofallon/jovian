...
def sw_affine(restrict_turns=True, 
  ...
  def sco(x, lengths, gap=0.0, open=0.0, temp=1.0):
    ...
    # sink
    smin = hij + x[1:, 1:, None]
    val = _soft_maximum(hij + x[1:,1:,None], mask=mask[1:,1:,None])
    return val
...
class SmithWatermanLoss(nn.Module):
  ...
  def __init__(self, gap_open_penalty, gap_extend_penalty, temperature=1.0, penalize_turns=True, restrict_turns=True):
      super().__init__()
  ...
  def _rotate(self, x):
      ...
      y[i, j] = x
      output = (y, (torch.arange(n) + x.shape[0] % 2) % 2)
      return output, \
  ...
  def _step(self, prev, smx, smo):
      ...
      right = self._cond(smo, torch.nn.functional.pad(h1[:-1], (0, 0, 1, 0), value=self.neg_inf), h1)
      down = self._cond(smo, h1, torch.nn.functional.pad(h1[1:], (0, 0, 0, 1), value=self.neg_inf))

      # add gap penalty
      if self.penalize_turns:
          right += torch.tensor([self.gap_open_penalty, self.gap_extend_penalty, self.gap_open_penalty])
          down += torch.tensor([self.gap_open_penalty, self.gap_open_penalty, self.gap_extend_penalty])
      else:
          gap_pen = torch.tensor([self.gap_open_penalty, self.gap_extend_penalty, self.gap_extend_penalty])
          right += gap_pen
          down += gap_pen
  ...
  def forward(self, x):
      ...
      for i in range(smx.shape[0]):
          carry, y = self._step(carry, smx[i, :], smo[i,])
          results.append(y)
      results = torch.stack(results)
      hij = results[ij_jx]
...
def test():
  ...
  full_result = sw_func(jnp.array(inputmat), (inputmat.shape[0], inputmat.shape[1]), open=-5, gap=-1)


  t = torch.tensor(inputmat, dtype=torch.float, requires_grad=True)
  swloss = SmithWatermanLoss(gap_open_penalty=-5, gap_extend_penalty=-1, temperature=1.0)
  pytresult = swloss(t)
  pytresult.backward()
