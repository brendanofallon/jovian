...
def aln_to_vars(refseq, altseq, chrom, offset=0, probs=None):
    ...
    if probs is not None:
        assert len(probs) == len(
            altseq
        ), f"Probabilities must contain same number of elements as alt sequence"
    else:
    ...
    if aln.query_begin > 0:
        q_offset += (
            aln.query_begin
        )  # Maybe we don't want this? query is alt sequence, so nonzero indicates first alt base matches downstream of first ref base
...
def construct_vcfvars(vars_hap0, vars_hap1, aln, reference, mindepth=30):
    ...
    for var in homs:
        ...
        vcfvars_hap0[var].qual = (
            vcfvars_hap0[var].qual + vcfvars_hap1[var].qual
        ) / 2  # Mean of each call??
        vcfvars_hap0[var].quals = (
            vcfvars_hap0[var].quals + vcfvars_hap1[var].quals
        )  # combine haps
        vcfvars_hap0[var].window_cis_vars = min(
            vcfvars_hap0[var].window_cis_vars, vcfvars_hap1[var].window_cis_vars
        )  # cis and trans are now cis
        vcfvars_hap0[var].window_trans_vars = 0  # moved all vars to cis
        vcfvars_hap0[var].call_count += vcfvars_hap1[
            var
        ].call_count  # combine call counts in both haplotypes
        vcfvars_hap0[var].genotype = (1, 1)
        vcfvars_hap0[var].het = False
        vcfvars_hap0[var].window_offset = sorted(
            set(vcfvars_hap0[var].window_offset + vcfvars_hap1[var].window_offset)
        )
    ...
    # combine haplotypes
    assert (
        len(set(vcfvars_hap0) & set(vcfvars_hap1)) == 0
    ...
    for key, var in vcfvars.items():
        ...
        if var.ref == "" or any(v == "" for v in var.alts):
            leading_ref_base = reference.fetch(
                reference=var.chrom, start=var.pos - 2, end=var.pos - 1
            )
            var.pos = var.pos - 1
...
def create_vcf_header(sample_name="sample", lowcov=30, cmdline=None):
    ...
    vcfh.add_meta(
        ...
        items=[
            ...
            (
                "Description",
                "Number of model haplotype calls of this var in multi-step detection",
            ),
        ],
    )
    vcfh.add_meta(
        ...
        items=[
            ...
            (
                "Description",
                "Number of overlapping steps where var detected in multi-step detection",
            ),
        ],
    )
    vcfh.add_meta(
...
def create_vcf_rec(var, vcf_file):
    ...
    r = vcf_file.new_record(
        contig=var.chrom,
        start=var.pos - 1,
        stop=var.pos - 1 + len(var.ref),
        alleles=(var.ref, *var.alts),
        filter=vcf_filter,
        qual=int(round(prob_to_phred(var.qual))),
    )
    # Set FORMAT values
    r.samples["sample"]["GT"] = var.genotype
    r.samples["sample"].phased = (
        var.phased
    )  # note: need to set phased after setting genotype
