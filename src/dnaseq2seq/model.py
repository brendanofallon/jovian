...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        self, d_model: int, dropout: float = 0.1, max_len: int = 500, batch_first=False
    ):
        super().__init__()
...
class VarTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.tgt_pos_encoder = PositionalEncoding(
            self.kmer_dim, batch_first=True, max_len=500
        ).to(self.device)
        ...
        self.encoder = nn.TransformerEncoder(
            encoder_layers, num_layers=n_encoder_layers
        )
        ...
        self.decoder0 = nn.TransformerDecoder(
            decoder_layers, num_layers=n_decoder_layers
        )
    ...
    def encode(self, src):
        ...
        src = self.pos_encoder(
            src
        )  # For 2D encoding we have to do this before flattening, right?
    ...
    def decode(self, mem, tgt, tgt_mask, tgt_key_padding_mask=None):
        ...
        h0 = self.decoder0(
            tgt0, mem_proj, tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask
        )
        h1 = self.decoder1(
            tgt1, mem_proj, tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask
        )
        h0 = self.softmax(h0)
        h1 = self.softmax(h1)
    ...
    def forward(self, src, tgt, tgt_mask, tgt_key_padding_mask=None):
        mem = self.encode(src)
        result = self.decode(
            mem, tgt, tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask
        )
        return result
