...
USE_DDP = (
    int(os.environ.get("RANK", -1)) >= 0 and os.environ.get("WORLD_SIZE") is not None
)
...
class TrainLogger:
    ...
    def log(self, items):
        assert len(items) == len(
            self.headers
        ), f"Expected {len(self.headers)} items to log, but got {len(items)}"
        self.output.write(",".join(str(items[k]) for k in self.headers) + "\n")
        self._flush_and_fsync()
...
def compute_twohap_loss(preds, tgt, criterion):
    ...
    with torch.no_grad():
        ...
        for b in range(preds.shape[0]):
            loss1 = criterion(
                preds[b, :, :, :].flatten(start_dim=0, end_dim=1),
                tgt[b, :, :].flatten(),
            )
            loss2 = criterion(
                preds[b, :, :, :].flatten(start_dim=0, end_dim=1),
...
def train_n_samples(
    ...
    for batch, (src, tgt_kmers, tgtvaf, altmask, log_info) in enumerate(loader_iter):
        ...
        if batch % 10 == 0:
            ...
            logger.info(
                f"Batch {batch}  samples: {samples_seen}   loss: {loss.item():.3f}   swaps: {swaps}   samples/sec: {samples_per_sec :.2f}"
            )
            start = time.perf_counter()
            samples_perf = 0
        ...
        if lr_schedule and batch % 10 == 0:
            ...
            logger.info(
                f"LR samples seen: {lr_schedule.iters}, learning rate: {lr_schedule.get_last_lr() :.6f}"
            )
...
def _calc_hap_accuracy(src, seq_preds, tgt, result_totals):
    ...
    match = (
        (
            torch.argmax(seq_preds[:, :, :].flatten(start_dim=0, end_dim=1), dim=1)
            == tgt[:, :].flatten()
        )
        .float()
        .mean()
    ...
    for b in range(src.shape[0]):
        ...
        vc = len(
            list(vcf.aln_to_vars(tgtstr, predstr, chrom="X"))
        )  # chrom arbitrary, required for building variant objs
        var_count += vc

        # Get TP, FN and FN based on reference, alt and predicted sequence.
        vartype_count = eval_prediction(
            util.readstr(src[b, :, 0, :]),
            tgtstr,
            seq_preds[b, :, 0 : util.KMER_COUNT],
...
def calc_val_accuracy(loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for src, tgt_kmers, vaf, *_ in loader.iter_once(64):
            ...
            seq_preds, probs = util.predict_sequence(
                src, model, n_output_toks=37, device=DEVICE
            )  # 150 // 4 = 37, this will need to be changed if we ever want to change the output length
            ...
            seq_preds = seq_preds[
                :, :, 0:j, :
            ]  # tgt_kmer_idx might be a bit shorter if the sequence is truncated

            loss, swaps = compute_twohap_loss(seq_preds, tgt_kmer_idx, criterion)
            loss_tot += loss
            swap_tot += swaps

            midmatch0, varcount0, results_totals0 = _calc_hap_accuracy(
                src, seq_preds[:, 0, :, :], tgt_kmer_idx[:, 0, :], result_totals0
            )
            midmatch1, varcount1, results_totals1 = _calc_hap_accuracy(
                src, seq_preds[:, 1, :, :], tgt_kmer_idx[:, 1, :], result_totals1
            )
...
def load_model(modelconf, ckpt):
    ...
    if ckpt is not None:
        ...
        if "conf" in ckpt:
            logger.warning(
                f"Found model conf AND a checkpoint with model conf - using the model params from checkpoint"
            )
            modelconf = ckpt["conf"]
...
def train_epochs(
    ...
    trainlogpath = (
        str(model_dest).replace(".model", "").replace(".pt", "") + "_train.log"
    )
    ...
    if val_dir:
        ...
        val_loader = loader.PregenLoader(
            device=DEVICE,
            datadir=val_dir,
            max_decomped_batches=4,
            threads=8,
            tgt_prefix="tgkmers",
        )
    else:
        logger.info(
            f"No val. dir. provided retaining a few training samples for validation"
        )
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            assert (
                samples_per_epoch > 0
            ), "Must have positive number of samples per epoch"
            loss = train_n_samples(
                model, optimizer, criterion, sample_iter, samples_per_epoch, scheduler
            )
            ...
            if MASTER_PROCESS:
                ...
                logger.info(
                    f"Epoch {epoch} Secs: {elapsed.total_seconds():.2f} lr: {scheduler.get_last_lr():.5f} loss: {loss:.4f} val acc: {acc0:.3f} / {acc1:.3f}  ppa: {ppa_snv:.3f} / {ppa_ins:.3f} / {ppa_dels:.3f}  ppv: {ppv_snv:.3f} / {ppv_ins:.3f} / {ppv_dels:.3f} swaps: {swaps}"
                )
                trainlogger.log(
                    {
                        "epoch": epoch,
                        "trainingloss": loss,
                        "val_accuracy": (
                            acc0.item() if isinstance(acc0, torch.Tensor) else acc0
                        ),
...
def train(output_model, **kwargs):
    ...
    if USE_DDP:
        logger.info(
            f"Using DDP: Master addr: {os.environ['MASTER_ADDR']}, port: {os.environ['MASTER_PORT']}, global rank: {os.environ['RANK']}, world size: {os.environ['WORLD_SIZE']}"
        )
        if MASTER_PROCESS:
        ...
        logger.info(
            f"DDP [{os.getpid()}] CUDA device {DEVICE} name: {torch.cuda.get_device_name()}"
        )
    else:
        ...
        if "cuda" in str(DEVICE):
            for idev in range(torch.cuda.device_count()):
                logger.info(
                    f"CUDA device {idev} name: {torch.cuda.get_device_name({idev})}"
                )
        DEVICE = (
            torch.device("cuda")
    ...
    dataloader = loader.PregenLoader(
        DEVICE,
        kwargs.get("datadir"),
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=kwargs.get("learning_rate", 0.001), betas=(0.9, 0.999)
    )
