...
def gensus(args):
    ...
    for line in open(input_bed).readlines():
        ...
        if region_type == "tn-map" or (region_type == "tn" and random.random() < 0.2): # Important update 12/2023 - add tn-map as a region to generate sus labels for, since many, many FPs come from tn-map areas
