...
class PositionalEncoding2D(nn.Module):
    ...
    def __init__(self, channels, device, x, y):
        """
    ...
    def _gen_mat(self, x, y, device, type):
        ...
        pos_y = torch.arange(y, device=device).type(self.inv_freq.type())
        sin_inp_x = torch.einsum("i,j->ij", pos_x, self.inv_freq)
        sin_inp_y = torch.einsum("i,j->ij", pos_y, self.inv_freq)
        emb_x = torch.cat((sin_inp_x.sin(), sin_inp_x.cos()), dim=-1).unsqueeze(1)
        emb_y = torch.cat((sin_inp_y.sin(), sin_inp_y.cos()), dim=-1)
        emb = torch.zeros((x, y, self.channels * 2), device=device).type(type)
    ...
    def forward(self, tensor):
        ...
        orig_ch = tensor.shape[3]
        if tensor.get_device() > -1:
            emb = self.emb.to(tensor.get_device())
        else:
            emb = self.emb
...
class VarTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        logger.info(f"Creating VarTransformer with read depth: {read_depth}")
        self.converter = nn.Linear(self.embed_dim, self.kmer_dim)
        self.pos_encoder = PositionalEncoding2D(self.fc1_hidden, self.device, 150, read_depth)
