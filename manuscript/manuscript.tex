...
To characterize variant detection accuracy, we called variants on validation regions from chromosomes 21 and 22 for each of the 17 samples, and compared results to DeepVariant (v1.3.0, Poplin et al. 2018), HaplotypeCaller (v. 4.1.4.1), and Strelka (v. 2.9.10, Kim et al. 2018). Calls were obtained with the 10M model trained for 25 epochs on the 'bigger' training set (~$10^6$ training examples). We selected filtering criteria for each caller by reviewing ROC curves produced by \textit{vcfeval} and selecting values close to the $F_1$ maximizing value across samples. Jovian calls were filtered at 0.4, HaplotypeCaller at 60 (phred-scaled), Strelka at 4 (phred-scaled), and DeepVariant at 0. Sensitivity, precision, and related calculations were computed with the hap.py tool (Krushe et al. 2019) using Genome-in-a-Bottle (v.4.2.1) as the truth set of variants. 

Across all regions, Jovian's sensitivity and precision for indels is slightly higher than that for HaplotypeCaller, and slightly lower than DeepVariant or Strelka2. For SNV calls Jovian has middling sensitivity and the lowest precision of the callers. 
...
While our model has comparable sensitivity to the other callers we examined, our specificity for SNV calls is lower. Strategies to improve SNV specificity include adding additional training data with false positive SNV calls, adding new features to the quality-estimation classifier, or possibly running an ensemble of models. 
